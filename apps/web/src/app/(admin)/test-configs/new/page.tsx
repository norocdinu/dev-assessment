...
export default function NewTestConfigPage() {
  ...
  return (
    <div className="p-6">
      <h2 className="text-lg font-semibold text-foreground mb-6">New Test Configuration</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 max-w-lg">
        <div>
          ...
          <input type="text" value={values.name} onChange={set('name')} className="w-full px-3 py-2 border border-border rounded-md text-sm" placeholder="e.g. Power BI Mid Test" />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1">Number of Questions</label>
            <input type="number" min={1} max={100} value={values.num_questions} onChange={set('num_questions')} className="w-full px-3 py-2 border border-border rounded-md text-sm" />
            {errors.num_questions && <p className="text-xs text-red-600 mt-1">{errors.num_questions}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1">Pass Threshold %</label>
            <input type="number" min={1} max={100} value={values.pass_threshold_pct} onChange={set('pass_threshold_pct')} className="w-full px-3 py-2 border border-border rounded-md text-sm" />
            {errors.pass_threshold_pct && <p className="text-xs text-red-600 mt-1">{errors.pass_threshold_pct}</p>}
          </div>
        </div>
        <button type="submit" disabled={loading} className="px-6 py-2 bg-[var(--brand)] text-white text-sm font-medium rounded-md hover:bg-[var(--brand)]/90 disabled:opacity-50">
